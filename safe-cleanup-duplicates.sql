-- 🔧 SAFE CLEANUP DUPLICATE USERS (PRESERVING FOREIGN KEYS)
...
-- 4. Update users table with unique data (instead of delete/insert)
UPDATE users 
SET 
    email = unique_users.email,
    phone = unique_users.phone,
    created_at = unique_users.created_at,
    updated_at = unique_users.updated_at
FROM unique_users
WHERE users.id = unique_users.id;

-- 5. Delete only the duplicate rows (keeping one per user)
DELETE FROM users 
WHERE id IN (
    SELECT id FROM (
...
-- ✅ DONE! Duplicate users should now be cleaned up safely.
